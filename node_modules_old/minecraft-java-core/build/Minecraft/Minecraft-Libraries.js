...
class Libraries {
    ...
    async natives(bundle) {
        ...
        for (const native of natives) {
            const entries = await (0, Index_js_1.getFileFromArchive)(native, null, null, true);
            for (const entry of entries) {
                if (entry.name.startsWith('META-INF'))
                    continue;
                if (entry.isDirectory) {
                    fs_1.default.mkdirSync(`${nativesFolder}/${entry.name}`, { recursive: true, mode: 0o777 });
                    continue;
                }
                // Write the file to the natives folder
                fs_1.default.writeFileSync(`${nativesFolder}/${entry.name}`, entry.data, { mode: 0o777 });
            }
        }
        return natives;
    }
